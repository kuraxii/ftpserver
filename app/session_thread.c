...
int send_by_cmd(void *_self, char *response) // 以cmd套接字发送数据
{
    ...
    printf("send: %s\n", response);
    // // write(self->cmdSocket->socketFd, response, strlen(response));
    log_info("send by cmd success");
